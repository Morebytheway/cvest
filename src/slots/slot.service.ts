...
//    async testCache() {
//     await this.cacheManager.set('test_slot', { name: 'Cvest ' }, 60);
...
export class SlotService implements OnModuleInit {
  ...
  constructor(
    ...
    @InjectModel(SlotSettings.name)
    private readonly settingsModel: Model<SlotSettingsDocument>,
    @Inject(CACHE_MANAGER) private readonly cacheManager: Cache,
    private readonly cacheService: CacheService,
  ) {}
  ...
  async updateGlobalSettings(updates: Partial<SlotSettings>, user: any) {
    ...
    if (typeof updates.globalEnabled === 'boolean') {
      ...
      await this.slotModel.updateMany(
        {},
        { $set: { isActive: updates.globalEnabled } },
      );
    }
    ...
    if (typeof updates.defaultAmount === 'number') {
      ...
      await this.slotModel.updateMany(
        {},
        { $set: { amount: updates.defaultAmount } },
      );
    }
    ...
    if (Array.isArray(updates.slotsPerDay)) {
      ...
      await this.syncSlotsWithSettings(
        updates.slotsPerDay,
    ...
    if (updates.slotAmounts) settings.slotAmounts = updates.slotAmounts;
    if (updates.dateOverrides) settings.dateOverrides = updates.dateOverrides;
  ...
  async updateSingleSlot(
    ...
    if (!slot) {
      ...
      if (
        updates.status &&
        ['available', 'booked', 'unavailable'].includes(updates.status)
      ) {
  ...
  private async syncSlotsWithSettings(
    slotsPerDay: string[],
  ...
  async toggleSlotTime(time: string, value: boolean, user: any) {
    ...
    const settings =
      (await this.settingsModel.findOne()) ?? new this.settingsModel();
  ...
  async removeSlotTimeFromMaster(time: string, user: any) {
    ...
    const settings =
      (await this.settingsModel.findOne()) ?? new this.settingsModel();
    settings.slotsPerDay = (settings.slotsPerDay || []).filter(
      (t) => t !== time,
    );
  ...
  private assembleMemorySlotsForDate(date: string, settings: any) {
    ...
    return slots.map((time: string) => {
      ...
      const isActive =
        (dateOverride?.isActive ?? perTimeToggle ?? true) &&
  ...
  async remove(date: string, startTime: string, user: any) {
    ...
    if (!slot) throw new NotFoundException('Slot not found');
    if (slot.status === 'booked')
      throw new ForbiddenException('Cannot delete booked slot');
