...
export class PaymentsService {
  ...
  async initiatePayment(bookingId: string, userEmail: string) {
    try {
      ...
      if (!userEmail) {
        throw new BadRequestException('Email is required for payment');
      }
  ...
  async verifyPayment(
    ...
    try {
      ...
      if (emailSent) {
        const payload = {
          teamName:
            booking.teamName || (booking.user as any)?.name || 'Guest Team',
        ...
        this.mailService
          .sendTicket((booking.user as any).email, payload)
          .catch(console.error);
      }
